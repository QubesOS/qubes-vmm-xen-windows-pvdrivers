...
XenNet_PutCbOnRing(struct xennet_info *xi, PVOID coalesce_buf, ULONG length, grant_ref_t gref)
{
  ...
  tx->gref = xi->vectors.GntTbl_GrantAccess(xi->vectors.context, xi->backend_id, (ULONG)(MmGetPhysicalAddress(coalesce_buf).QuadPart >> PAGE_SHIFT), FALSE, gref, (ULONG)'XNTX');
...
*/
  ...
  while (remaining > 0)
  {
    ...
    else
    {
      ...
      txN->gref = xi->vectors.GntTbl_GrantAccess(xi->vectors.context, xi->backend_id, (ULONG)pfn, FALSE, gref, (ULONG)'XNTX');

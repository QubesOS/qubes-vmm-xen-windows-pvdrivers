...
put_free_entry(WDFDEVICE Device, grant_ref_t ref)
{
  ...
  deviceData->gnttab_list[ref] = deviceData->gnttab_list[0];
  deviceData->gnttab_list[0]  = ref;
}
...
{
  PXENPCI_DEVICE_DATA deviceData = GetDeviceData(Device);
  unsigned int ref = deviceData->gnttab_list[0];

  deviceData->gnttab_list[0] = deviceData->gnttab_list[ref];
  return ref;
}
...
GntTab_Map(WDFDEVICE Device, unsigned int start_idx, unsigned int end_idx)
{
  ...
  do {
    ...
    xatp.gpfn = (xen_pfn_t)(deviceData->gnttab_table_physical.QuadPart >> PAGE_SHIFT) + i;
...
GntTbl_Init(WDFDEVICE Device)
{
  ...
  deviceData->gnttab_table_physical = XenPCI_AllocMMIO(Device,
    PAGE_SIZE * NR_GRANT_FRAMES);
  deviceData->gnttab_table = MmMapIoSpace(deviceData->gnttab_table_physical,
    PAGE_SIZE * NR_GRANT_FRAMES, MmNonCached);
  if (!deviceData->gnttab_table)
  {
  ...
  int readonly)
{
  ...
  deviceData->gnttab_table[ref].frame = frame;
  deviceData->gnttab_table[ref].domid = domid;
  //_WriteBarrier();
  KeMemoryBarrier();
  readonly *= GTF_readonly;
  deviceData->gnttab_table[ref].flags = GTF_permit_access | (uint16_t)readonly;
  ...
  grant_ref_t ref)
{
  ...
  nflags = deviceData->gnttab_table[ref].flags;
  do {
    ...
    (volatile SHORT *)&deviceData->gnttab_table[ref].flags, flags, 0)) != flags);

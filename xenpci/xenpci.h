...
#define DEBUGF(format, ...) DbgPrintEx(DPFLTR_IHVBUS_ID, DPFLTR_INFO_LEVEL, \
    "[" __DRIVER_NAME "] " __FUNCTION__ ": " format "\n", ##__VA_ARGS__)
...
NTSTATUS EvtChn_DeviceFileInit(WDFDEVICE device, PWDF_IO_QUEUE_CONFIG queue_config, WDFFILEOBJECT file_object);
...
char *XenBus_AddWatch(PVOID Context, xenbus_transaction_t xbt, char *Path, PXENBUS_WATCH_CALLBACK ServiceRoutine, PVOID ServiceContext);
char *XenBus_RemWatch(PVOID Context, xenbus_transaction_t xbt, char *Path, PXENBUS_WATCH_CALLBACK ServiceRoutine, PVOID ServiceContext);
//VOID
...
PHYSICAL_ADDRESS XenPci_AllocMMIO(PXENPCI_DEVICE_DATA xpdd, ULONG len);
...
BOOLEAN EvtChn_Test_Masked(PVOID Context, evtchn_port_t Port);
NTSTATUS EvtChn_Mask(PVOID Context, evtchn_port_t Port);
NTSTATUS EvtChn_Unmask(PVOID Context, evtchn_port_t Port);
NTSTATUS EvtChn_Reset(PVOID Context, evtchn_port_t Port);
NTSTATUS EvtChn_Bind(PVOID Context, evtchn_port_t Port, PXEN_EVTCHN_SERVICE_ROUTINE ServiceRoutine, PVOID ServiceContext, ULONG flags);
NTSTATUS EvtChn_BindDpc(PVOID Context, evtchn_port_t Port, PXEN_EVTCHN_SERVICE_ROUTINE ServiceRoutine, PVOID ServiceContext, ULONG flags);
NTSTATUS EvtChn_BindDpcReplace(PVOID Context, evtchn_port_t Port, PXEN_EVTCHN_SERVICE_ROUTINE ServiceRoutine, PVOID ServiceContext, ULONG flags, BOOLEAN replace, BOOLEAN mask_on_fire);
NTSTATUS EvtChn_BindIrq(PVOID Context, evtchn_port_t Port, ULONG vector, PCHAR description, ULONG flags);
evtchn_port_t EvtChn_AllocIpi(PVOID context, ULONG vcpu);

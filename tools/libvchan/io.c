...
static int do_send(struct libxenvchan *ctrl, const void *data, size_t size)
{
	...
	memcpy((char*)wr_ring(ctrl) + real_idx, data, avail_contig);
	if (avail_contig < size)
	{
		// we rolled across the end of the ring
		memcpy(wr_ring(ctrl), (const char*)data + avail_contig, size - avail_contig);
	}
	...
	if (ctrl->blocking) {
		...
		while (1) {
			...
			if (pos + avail > size)
				avail = size - pos;
			if (avail)
				pos += do_send(ctrl, (const char*)data + pos, avail);
			if (pos == size)
				return pos;
			if (libxenvchan_wait(ctrl))
				return -1;
...
static int do_recv(struct libxenvchan *ctrl, void *data, size_t size)
{
	...
	memcpy(data, (char*)rd_ring(ctrl) + real_idx, avail_contig);
	if (avail_contig < size)
	{
		// we rolled across the end of the ring
		memcpy((char*)data + avail_contig, rd_ring(ctrl), size - avail_contig);
	}
...
int
#endif
libxenvchan_fd_for_select(struct libxenvchan *ctrl)
{
...
void libxenvchan_close(struct libxenvchan *ctrl)
{
	...
	if (ctrl->event) {
		if (ctrl->event_port != -1 && ctrl->ring)
